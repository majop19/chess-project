...
const PasswordCriteria = ({ password }: { password: string }) => {
  const criteria = [
    { label: "At least 7 characters", met: password.length >= 7 },
  ...
  return (
    <div className="mt-2 space-y-1">
      {criteria.map((item) => (
        <div key={item.label} className="flex items-center text-xs font-bold">
          {item.met ? (
            <Check className="size-4 text-primary mr-2" />
          ) : (
            <X className="size-4 text-secondary mr-2" />
          )}
          <span className={item.met ? "text-primary" : "text-secondary"}>
            {item.label}
          </span>
        </div>
      ))}
        ...
        {[...Array(5)].map((_, index) => (
          <div
            ...
            className={`h-1 w-1/5 rounded-full transition-colors duration-300 
                ${index < strength ? getColor(strength) : "bg-foreground"}
              `}
          />
        ))}
      </div>
